...
type
  TfrCipher_line_by_line = class(TForm)
    Panel1: TPanel;
    lKljuc: TLabel;
  ...
  public
    ...
    procedure print_state(round : integer; name : string; explain : string);
...
var
  frCipher_line_by_line: TfrCipher_line_by_line;
...
//------------------------------------------------------------
procedure TfrCipher_line_by_line.print_state(round : integer; name : string; explain : string);
var i, j : integer;
    s, rez : string;
begin
...
//------------------------------------------------------------
procedure TfrCipher_line_by_line.print_round_key(key_index : byte; round : integer);
var i : integer;
    s : string;
    start : integer;
begin
  start := 16;
  key_index := start + key_index;
  s := '';
  for i := 0 to 15 do
    s := s + IntToHex(key[ key_index + i ], 2);
  Memo1.Lines.Add('round[' + Format('%2d', [round]) + '].' + 'k_sch' + '     ' + s + '    ---------- Part of expanded key on pos.' + IntToStr(key_index) + ' (16 bytes)')

end;

//------------------------------------------------------------
procedure TfrCipher_line_by_line.RotWord;
var a, c : Byte;
begin
		a := tmpkey[0];
		for c := 0 to 3 do
...
//------------------------------------------------------------
function TfrCipher_line_by_line.rcon (a : Byte) : Byte;
var c : Byte;
begin
  c := 1;
  if a = 0 then
...
//------------------------------------------------------------
// multiplication
function TfrCipher_line_by_line.galoa_mul_tab(a : Byte; b : Byte) : Byte;
var s, z : integer;
begin
  z := 0;
...
//------------------------------------------------------------
// MixColumns
procedure TfrCipher_line_by_line.MixColumns;
var c : Byte;
    a : array [0..3] of BYTE;
begin
...
//------------------------------------------------------------
// ShiftRows
procedure TfrCipher_line_by_line.ShiftRows;
...
//------------------------------------------------------------
procedure TfrCipher_line_by_line.Cipher(key_len : Integer);
var i, j : integer;
...
begin
    ...
    print_state(0, 'input', 'Buffer state after input');
    print_round_key(0, 0);
    Round2 := 0;
    ...
    begin
      print_state(round1, 'start', '********** Buffer STATE on start of round');

      // SubBytes
      for i := 0 to 3 do
        for j := 0 to 3 do
          state[i, j] := sbox[ state[i, j] ];
      print_state(round1, 's_box', 'After SubBytes (S-Box)');

      ShiftRows;
      print_state(round1, 's_row', 'After ShiftRows');

      MixColumns;
      print_state(round1, 'm_col', 'After MixColumns');
      print_round_key(4*round1*Nb, round1);

      AddRoundKey( 4*round1*Nb );  // Nb - No.of columns = 4
      print_state(round1, 'k_add', 'After AddRoundKey (this is input for next round)');
    ...
    print_state(round2+1, 'start', '********** Buffer STATE on start of FINAL round');
    ...
    for i := 0 to 3 do
      for j := 0 to 3 do
       state[i, j] := sbox[ state[i, j] ];
    print_state(round2+1, 's_box', 'After SubBytes (S-Box)');

    ShiftRows;
    print_state(round2+1, 's_row', 'After ShiftRows');
    print_round_key(4*round2*Nb, round2+1);

    inc(round2);
    AddRoundKey( 4*round2*Nb  );
    print_state(round2, 'k_add', 'After AddRoundKey');

    Memo1.Lines.Add('--------------- Final round end --------------------');
    print_state(round2, 'out  ', 'This is result');
    Memo1.Lines.Add('------------------------------------------------------------------------------------');
...
//------------------------------------------------------------
// key_exp_base
procedure TfrCipher_line_by_line.key_exp_base(i : Byte);
var a : Byte;
begin
		// RotWord
		//---------------------    ---------------------
...
procedure TfrCipher_line_by_line.KeyExpansion128;
var c, i, a : Byte;
begin
...
procedure TfrCipher_line_by_line.KeyExpansion192;
var c, i, a : Byte;
begin
...
procedure TfrCipher_line_by_line.KeyExpansion256;
var c, i, a : Byte;
begin
...
//------------------------------------------------------------
procedure TfrCipher_line_by_line.AddRoundKey(key_index : byte);
var i, j : Byte;
begin
  for j := 0 to 3 do
    for i := 0 to 3 do
...
procedure TfrCipher_line_by_line.rgTestVektoriClick(Sender: TObject);
begin
if rgTestVektori.ItemIndex = 0 then
begin
  rgAlgoritam.ItemIndex := 0;
  Kljuc.Text := '2b7e151628aed2a6abf7158809cf4f3c';
  UlazniVektor.Text := '3243f6a8885a308d313198a2e0370734';
  Rezultat.Text := '';
end
else
...
begin
  ...
  UlazniVektor.Text := '00112233445566778899aabbccddeeff';
  Rezultat.Text := '';
end
else
...
procedure TfrCipher_line_by_line.btCipherClick(Sender: TObject);
...
procedure TfrCipher_line_by_line.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action := caFree;
end;

procedure TfrCipher_line_by_line.btClearClick(Sender: TObject);
begin
  Memo1.Lines.Clear;
end;
