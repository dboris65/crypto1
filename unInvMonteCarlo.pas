...
uses
  ...
  // here are predefined test vectors
  unInvMonteCarloVars;

type
  TfrInvMonteCarlo = class(TForm)
    Panel1: TPanel;
    lKljuc: TLabel;
...
var
  frInvMonteCarlo: TfrInvMonteCarlo;
...
//------------------------------------------------------------
procedure TfrInvMonteCarlo.RotWord;
var a, c : Byte;
begin
		a := tmpkey[0];
		for c := 0 to 3 do
...
//------------------------------------------------------------
function TfrInvMonteCarlo.rcon (a : Byte) : Byte;
var c : Byte;
begin
  c := 1;
  if a = 0 then
...
//------------------------------------------------------------
// multiplication
function TfrInvMonteCarlo.galoa_mul_tab(a : Byte; b : Byte) : Byte;
var s, z : integer;
begin
  z := 0;
...
//------------------------------------------------------------
// InvMixColumns
procedure TfrInvMonteCarlo.InvMixColumns;
var c : Byte;
    a : array [0..3] of BYTE;
begin
// 0e 0b 0d 09
...
//------------------------------------------------------------
// InvShiftRows
procedure TfrInvMonteCarlo.InvShiftRows;
...
//------------------------------------------------------------
procedure TfrInvMonteCarlo.InvCipher(key_len : Integer);
var i, j : integer;
    round : integer;
...
//------------------------------------------------------------
// key_exp_base
procedure TfrInvMonteCarlo.key_exp_base(i : Byte);
var a : Byte;
begin
		// RotWord
		//---------------------    ---------------------
...
procedure TfrInvMonteCarlo.KeyExpansion128;
var c, i, a : Byte;
begin
...
procedure TfrInvMonteCarlo.KeyExpansion192;
var c, i, a : Byte;
begin
...
procedure TfrInvMonteCarlo.KeyExpansion256;
var c, i, a : Byte;
begin
...
//------------------------------------------------------------
procedure TfrInvMonteCarlo.AddRoundKey(key_index : byte);
var i, j : Byte;
begin
  for j := 0 to 3 do
    for i := 0 to 3 do
...
procedure TfrInvMonteCarlo.btMonteCarloCipherClick(Sender: TObject);
...
begin
  ...
  if Trim(Rezultat.Text) <> Trim(cbPlains.Items[cbCiphers.ItemIndex]) then
...
procedure TfrInvMonteCarlo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action := caFree;
end;

procedure TfrInvMonteCarlo.rgAlgoritamClick(Sender: TObject);
var i : integer;
begin
if rgAlgoritam.ItemIndex = 0 then
begin
  ...
  UlazniVektor.Text := cipher_texts_128[cbKeys.ItemIndex];
end
else
if rgAlgoritam.ItemIndex = 1 then
begin
  ...
  UlazniVektor.Text := cipher_texts_192[cbKeys.ItemIndex];
end
else
if rgAlgoritam.ItemIndex = 2 then
begin
  ...
  UlazniVektor.Text := cipher_texts_256[cbKeys.ItemIndex];
end;
...
procedure TfrInvMonteCarlo.cbKeysChange(Sender: TObject);
begin
// predefined test vectors are in unInvMonteCarloVars.pas
if rgAlgoritam.ItemIndex = 0 then
begin
...
procedure TfrInvMonteCarlo.cbPlainsChange(Sender: TObject);
begin
// predefined test vectors are in unInvMonteCarloVars.pas
if rgAlgoritam.ItemIndex = 0 then
begin
...
procedure TfrInvMonteCarlo.cbCiphersChange(Sender: TObject);
begin
// predefined test vectors are in unInvMonteCarloVars.pas
if rgAlgoritam.ItemIndex = 0 then
begin
...
procedure TfrInvMonteCarlo.FormShow(Sender: TObject);
begin
...
procedure TfrInvMonteCarlo.btMonteCarloAllClick(Sender: TObject);
var ulaz  : array [0..15] of Byte;
    skljuc : array [0..31] of Byte;
    s, st : string;
    i, j, k, l : integer;
