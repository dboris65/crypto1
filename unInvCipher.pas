...
//------------------------------------------------------------
procedure TfrInvCipher.add_lines(round : integer);
begin
    ...
    memo_line1 := '        ';
    memo_line2 := '        ';
    memo_line2 := IntToStr(round) + memo_line2;
...
//------------------------------------------------------------
procedure TfrInvCipher.print_state;
var i : integer;
    s : string;
begin
  s := '|';
...
//------------------------------------------------------------
procedure TfrInvCipher.print_round_key(key_index : byte);
var i : integer;
    s : string;
begin
  s := '|';
  for i := 0 to 3 do
    s := s + IntToHex(key[key_index + 4*i + 0], 2) + ' ';
  memo_line1 := memo_line1 + s + '|  ';

  s := '|';
  for i := 0 to 3 do
    s := s + IntToHex(key[key_index + 4*i + 1], 2) + ' ';
  memo_line2 := memo_line2 + s + '|  ';

  s := '|';
  for i := 0 to 3 do
    s := s + IntToHex(key[key_index + 4*i + 2], 2) + ' ';
  memo_line3 := memo_line3 + s + '|  ';

  s := '|';
  for i := 0 to 3 do
    s := s + IntToHex(key[key_index + 4*i + 3], 2) + ' ';
  memo_line4 := memo_line4 + s + '|  ';
end;

//------------------------------------------------------------
procedure TfrInvCipher.RotWord;
var a, c : Byte;
begin
		a := tmpkey[0];
		for c := 0 to 3 do
...
//------------------------------------------------------------
function TfrInvCipher.rcon (a : Byte) : Byte;
var c : Byte;
begin
  c := 1;
  if a = 0 then
...
//------------------------------------------------------------
// multiplication
function TfrInvCipher.galoa_mul_tab(a : Byte; b : Byte) : Byte;
var s, z : integer;
begin
  z := 0;
...
//------------------------------------------------------------
// InvMixColumns
procedure TfrInvCipher.InvMixColumns;
var c : Byte;
    a : array [0..3] of BYTE;
begin
// 0e 0b 0d 09
...
//------------------------------------------------------------
// ShiftRows
procedure TfrInvCipher.InvShiftRows;
...
//------------------------------------------------------------
procedure TfrInvCipher.InvCipher(key_len : Integer);
var i, j : integer;
    round : integer;
...
begin
    ...
    Memo1.Lines.Add(' Round   Start of           After           After         Round key        After           After  ');
    Memo1.Lines.Add(' Number    Round           ShiftRows       SubBytes         Value        AddRoundKey      MixColums');
    Memo1.Lines.Add('------------------------------------------------------------------------------------------------------');
    print_state;
...
print_state;
memo_line2[1] := 'o'; memo_line2[2] := 'u'; memo_line2[3] := 't';
memo_line2[4] := 'p'; memo_line2[5] := 'u'; memo_line2[6] := 't';
add_lines(0);
Memo1.Lines.Add('------------------------------------------------------------------------------------------------------');
...
//------------------------------------------------------------
// key_exp_base
procedure TfrInvCipher.key_exp_base(i : Byte);
var a : Byte;
begin
		// RotWord
		//---------------------    ---------------------
...
procedure TfrInvCipher.KeyExpansion128;
var c, i, a : Byte;
begin
...
procedure TfrInvCipher.KeyExpansion192;
var c, i, a : Byte;
begin
...
procedure TfrInvCipher.KeyExpansion256;
var c, i, a : Byte;
begin
...
//------------------------------------------------------------
procedure TfrInvCipher.AddRoundKey(key_index : byte);
var i, j : Byte;
begin
  for j := 0 to 3 do
    for i := 0 to 3 do
...
procedure TfrInvCipher.rgTestVektoriClick(Sender: TObject);
begin
...
procedure TfrInvCipher.btCipherClick(Sender: TObject);
...
procedure TfrInvCipher.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action := caFree;
end;

procedure TfrInvCipher.FormShow(Sender: TObject);
begin
memo_line1 := '';
memo_line2 := '';
memo_line3 := '';
